...
<form method="POST"
      ...
      x-data="purchaseForm(@js($initialLines), @js((float) old('discount_amount', $purchase->discount_amount ?: 0)), @js((float) old('paid_amount', $purchase->paid_amount ?: 0)))"
      class="space-y-4">
    @csrf
        ...
        <div class="card">
            <div class="card-body space-y-2.5 text-sm">
                ...
                <div class="flex justify-between items-center" x-show="discount > 0">
                    ...
                    <span class="num font-semibold text-rose-600" x-text="'- ' + money(discount)">0 د.ع</span>
                </div>
                ...
                <div class="flex justify-between items-center text-emerald-700 font-semibold" x-show="paid > 0">
                    <span>پارەی دراو</span>
                    <span class="num" x-text="money(paid)">0 د.ع</span>
                </div>
...
function purchaseForm(initialLines, initialDiscount, initialPaid) {
    return {
        ...
        remaining() {
            return Math.max(0, this.total() - (parseFloat(this.paid) || 0));
        },
