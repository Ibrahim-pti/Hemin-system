...
<div class="space-y-4 sm:space-y-6">
    ...
    {{-- ٢. کارتە ئامارییەکانی دروستکردن --}}
    ...
    {{-- ٣. کۆی کەلوپەلە دروستکراوەکان بەپێی جۆر --}}
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
            <div class="p-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-black text-sm text-slate-800">پوختەی کەلوپەلە دروستکراوەکان بەپێی بابەت</h3>
                </div>
                ...
                <table class="w-full text-right text-xs">
                    <thead class="bg-slate-50 text-slate-600 border-b border-slate-200 font-black">
                        <tr>
                            ...
                            <th class="p-3.5">ناوی کەلوپەل / بەرهەم</th>
                            <th class="p-3.5 text-center">ژمارەی وەسڵەکان</th>
                            <th class="p-3.5 text-left">کۆی بڕی دروستکراو</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($itemsBreakdown as $index => $row)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="p-3.5 text-center font-mono font-bold text-slate-400">{{ $index + 1 }}</td>
                                <td class="p-3.5 font-black text-slate-900">{{ $row['name'] }}</td>
                                <td class="p-3.5 text-center">
                                    <span class="px-2.5 py-0.5 rounded-lg bg-slate-100 font-mono font-bold text-slate-700">
                                        {{ fmt_num($row['count']) }} وەسڵ
                                    </span>
                                </td>
                                <td class="p-3.5 text-left font-mono font-black text-indigo-700">
                                    {{ fmt_num($row['qty']) }} {{ $row['unit'] }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    {{-- ٤. خشتەی وردەکاری تەواوی وەسڵەکانی کارگە (بە وێنە و قیاسات) --}}
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div class="p-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <span class="text-base">📋</span>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-right text-xs">
                <thead class="bg-slate-50 text-slate-600 border-b border-slate-200 font-black">
                    <tr>
                        <th class="p-3.5 text-center w-20">وەسڵ</th>
                        <th class="p-3.5">کڕیار</th>
                        <th class="p-3.5">بەرواری داواکاری / گەیاندن</th>
                        <th class="p-3.5">کەلوپەل، وێنە و قیاسات بۆ دروستکردن</th>
                        <th class="p-3.5 text-center">دۆخی کارگە</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors align-top">
                                ...
                                <div class="space-y-2.5">
                                    @foreach($order->items as $it)
                                        @php
                                            $imgUrl = $it->imageUrl();
                                        @endphp
                                        <div class="flex items-center gap-3 p-2 rounded-xl bg-slate-50 border border-slate-200/80 shadow-2xs">
                                            {{-- وێنەی داواکاری / کەلوپەل --}}
                                            <div class="size-12 sm:size-14 rounded-xl bg-white border border-slate-200 shrink-0 overflow-hidden flex items-center justify-center">
                                                @if($imgUrl)
                                                    <a href="{{ $imgUrl }}" target="_blank" title="بینینی وێنەی گەورە">
                                                        <img src="{{ $imgUrl }}" alt="{{ $it->item_name }}"
                                                             class="size-12 sm:size-14 object-cover hover:scale-110 transition-transform cursor-pointer">
                                                    </a>
                                                @else
                                            ...
                                            {{-- وردەکاری قیاس و ناوی بابەت --}}
                                            <div class="min-w-0 flex-1">
                                                <div class="flex items-center justify-between gap-2 flex-wrap">
                                                    <span class="font-black text-xs text-slate-900">{{ $it->item_name }}</span>
                                                    <span class="font-mono font-black text-xs text-indigo-700 bg-white px-2 py-0.5 rounded-md border border-slate-200">
                                                        {{ fmt_num($it->quantity ?? $it->qty) }} {{ $it->unit_label ?: $it->unit_name }}
                                                    </span>
                                                </div>

                                                <div class="flex items-center gap-3 text-[11px] mt-1 text-slate-600 flex-wrap">
                                                    @if($it->width && $it->height)
                                                        <span class="font-mono font-bold text-indigo-800 bg-indigo-50 px-1.5 py-0.5 rounded">
                                                            پێوانە: {{ $it->width }} × {{ $it->height }} م
                                                        </span>
                                                    ...
                                                    @if($it->note)
                                                        <span class="text-slate-500 italic">({{ $it->note }})</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <td class="p-3.5 text-center">
                                @if($order->status === 'delivered')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 block">
                                        ڕادەستکراو ✔️
                                    </span>
                                ...
                                @elseif($order->status === 'in_production')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-indigo-100 text-indigo-800 border border-indigo-200 block animate-pulse">
                                        لە دروستکردندا ⚙️
                                    </span>
                                @elseif($order->status === 'confirmed')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-800 border border-amber-200 block">
                                        چاوەڕوانە ⏳
                                    </span>
                                @else
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200 block">
