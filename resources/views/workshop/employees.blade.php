...
@section('content')
<div x-data="workshopEmployeesApp()" class="space-y-6">
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-3.5">
            ...
            <div>
                <h1 class="text-xl font-black text-slate-800">تۆماری ئامادەبوون و وەستاکانی کارگە</h1>
        ...
        <div class="flex items-center gap-3 flex-wrap">
            <form method="GET" action="{{ route('workshop.employees') }}" class="flex items-center gap-2">
                ...
                <input type="date" name="date" value="{{ $date }}" onchange="this.form.submit()"
                       class="text-xs px-3 py-1.5 rounded-xl border border-slate-200 font-mono font-bold focus:outline-hidden focus:border-blue-500 bg-slate-50">
            </form>
            ...
            <div class="flex items-center gap-1.5 text-xs font-bold">
                ...
                <span class="px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 border border-rose-200">
                    نەهاتوو: {{ $absentCount }}
                </span>
            </div>
        </div>
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <template x-for="emp in employeesList" :key="emp.id">
                <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex flex-col justify-between hover:shadow-md transition-all">
                    <div>
                        {{-- بەشی سەرەوەی کارت --}}
                        <div class="flex items-start justify-between gap-2 border-b border-slate-100 pb-3 mb-3">
                            <div>
                                <div class="flex items-center gap-2">
                                    <h3 class="font-black text-slate-900 text-base" x-text="emp.name"></h3>
                                    <span class="px-2 py-0.5 rounded-lg text-[11px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200"
                            ...
                            <div>
                                ...
                                <span x-show="emp.attendance && emp.attendance.status === 'leave'"
                                    ...
                                    <span>🏖️</span>
                                    <span>ئیجازە / مۆڵەت</span>
                                </span>
                                <span x-show="emp.attendance && emp.attendance.status === 'absent'"
                                      class="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-200 flex items-center gap-1">
                                    <span>❌</span>
                                    <span>نەهاتووە</span>
                                </span>
                        ...
                        <div class="grid grid-cols-2 gap-2.5 mb-3 bg-slate-50/80 p-3 rounded-xl border border-slate-100">
                            <div>
                                <div class="text-[11px] font-bold text-slate-400 mb-0.5">کاتی هاتن (Check-in):</div>
                                <div class="text-sm font-black text-slate-800 font-mono" x-text="(emp.attendance && emp.attendance.check_in) ? emp.attendance.check_in : '—'"></div>
                            </div>
                            <div>
                                <div class="text-[11px] font-bold text-slate-400 mb-0.5">کاتی چوون (Check-out):</div>
                                <div class="text-sm font-black text-slate-800 font-mono" x-text="(emp.attendance && emp.attendance.check_out) ? emp.attendance.check_out : '—'"></div>
                            </div>
                        </div>
                    ...
                    <div class="border-t border-slate-100 pt-3 flex items-center justify-between gap-1.5 flex-wrap">
                        <div class="flex items-center gap-1.5">
                            ...
                            <button type="button" @click="quickCheckIn(emp.id)"
                                    class="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1 shadow-2xs cursor-pointer active:scale-95 transition-all">
                                <span>📥</span>
                                <span>چیک ئین</span>
                            </button>
                            ...
                            <button type="button" @click="quickCheckOut(emp.id)"
                                    class="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-900 text-white flex items-center gap-1 shadow-2xs cursor-pointer active:scale-95 transition-all">
                                <span>📤</span>
                                <span>چیک ئاوت</span>
                            </button>
                        </div>
                        ...
                        <button type="button" @click="openModalFor(emp)"
                                class="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 flex items-center gap-1 cursor-pointer transition-all">
                            <span>✏️</span>
    ...
    {{-- ============================================================ --}}
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-xs p-4">
        <div class="relative w-full max-w-lg bg-white rounded-3xl p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto" @click.outside="showModal = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                <div class="flex items-center gap-2.5">
                    <div class="size-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-lg border border-blue-100">
                        👷
                    </div>
                    <div>
                        <h3 class="font-black text-slate-800 text-base" x-text="'تۆماری کارمەند: ' + (selectedEmployee ? selectedEmployee.name : '')"></h3>
            ...
            <form @submit.prevent="saveAttendance" class="space-y-4 text-xs">
                ...
                {{-- کاتی هاتن و چوون --}}
                <div class="grid grid-cols-2 gap-3 bg-slate-50/80 p-3.5 rounded-2xl border border-slate-100">
                    <div>
                ...
                <div class="bg-amber-50/50 p-3.5 rounded-2xl border border-amber-200/60 space-y-2.5">
                ...
                <div class="bg-emerald-50/50 p-3.5 rounded-2xl border border-emerald-200/60 space-y-2.5">
