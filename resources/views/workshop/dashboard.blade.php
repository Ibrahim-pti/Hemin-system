...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('workshop.orders') }}" class="btn btn-primary !py-2 !px-4 text-xs font-bold bg-blue-600 hover:bg-blue-700 shadow-xs">
                <span>📋</span>
                <span>داواکارییەکانی کارگە</span>
            </a>
            <a href="{{ route('workshop.materials') }}" class="btn btn-ghost !py-2 !px-3 text-xs font-bold border border-slate-200 hover:bg-slate-50 text-slate-700">
                <span>📦</span>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <a href="{{ route('workshop.orders') }}?status=confirmed"
           class="bg-white rounded-2xl p-4.5 border border-slate-200 hover:border-amber-400 transition-all hover:shadow-md group">
            ...
            <div class="num text-2xl md:text-3xl font-black text-amber-600">{{ fmt_num($pendingCount) }}</div>
        ...
        <a href="{{ route('workshop.orders') }}?status=in_production"
           class="bg-white rounded-2xl p-4.5 border border-slate-200 hover:border-blue-400 transition-all hover:shadow-md group">
            <div class="flex items-center justify-between mb-2">
            ...
            <div class="num text-2xl md:text-3xl font-black text-blue-600">{{ fmt_num($inProductionCount) }}</div>
        ...
        <a href="{{ route('workshop.orders') }}?status=ready"
           class="bg-white rounded-2xl p-4.5 border border-slate-200 hover:border-emerald-400 transition-all hover:shadow-md group">
            <div class="flex items-center justify-between mb-2">
            ...
            <div class="num text-2xl md:text-3xl font-black text-emerald-600">{{ fmt_num($readyCount) }}</div>
        ...
        <a href="{{ route('workshop.orders') }}?status=delivered"
           class="bg-white rounded-2xl p-4.5 border border-slate-200 hover:border-slate-400 transition-all hover:shadow-md group">
            <div class="flex items-center justify-between mb-2">
            ...
            <div class="num text-2xl md:text-3xl font-black text-slate-800">{{ fmt_num($deliveredCount) }}</div>
        ...
        <div class="bg-gradient-to-r from-rose-50 via-amber-50 to-orange-50 rounded-2xl p-4.5 border border-rose-200 shadow-xs">
            <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
                <div class="flex items-center gap-2.5">
                    <span class="flex size-9 items-center justify-center rounded-xl bg-rose-600 text-white font-bold text-base shadow-xs">
                        ⚠️
                    </span>
                    <div>
                ...
                <a href="{{ route('workshop.materials') }}" class="text-xs font-bold text-rose-800 hover:text-rose-950 underline flex items-center gap-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="size-8 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-sm">
                            ⚙️
                        </span>
                        <div>
                            <h3 class="font-black text-sm text-slate-800">دوایین وەسڵە چالاکەکانی کارگە</h3>
                            <p class="text-[11px] text-slate-400 font-medium">ئەو کارانەی ئێستا لە دروستکردندا یان چاوەڕوانیدان</p>
                        </div>
                    </div>
                ...
                @else
                    <div class="divide-y divide-slate-100">
                        @foreach ($activeOrders as $order)
                            <div class="p-4 hover:bg-slate-50/80 transition-colors flex flex-wrap items-center justify-between gap-3">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-black text-sm text-slate-900">وەسڵی #{{ $order->invoice_no }}</span>
                                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold {{ $order->status === 'in_production' ? 'bg-blue-100 text-blue-800 border border-blue-200' : ($order->status === 'ready' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-amber-100 text-amber-800 border border-amber-200') }}">
                                            {{ $order->status_label }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="text-[11px] text-slate-400 mt-1 flex flex-wrap gap-2">
                                        ...
                                        @if ($order->items->count() > 3)
                                            <span class="text-slate-400 text-[10px]">+{{ $order->items->count() - 3 }} دانەی تر</span>
                                        @endif
                                    </div>
                                </div>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="size-8 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-sm">
                            👷
                        </span>
                        <div>
                            <h3 class="font-black text-sm text-slate-800">وەستاکانی کارگە</h3>
                            <p class="text-[11px] text-slate-400 font-medium">{{ $employees->count() }} کەسی چالاک</p>
                        </div>
                    </div>
                ...
                @else
                    <div class="divide-y divide-slate-100">
                        @foreach ($employees->take(5) as $emp)
                            ...
                            @endphp
                            <div class="p-3.5 hover:bg-slate-50 transition-colors flex items-center justify-between gap-2">
                                <div class="min-w-0">
